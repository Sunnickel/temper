...
pub struct ImprovedNoise {
    permutation_table: [u8; 256],
    pos: DVec3,
}

impl ImprovedNoise {
    pub fn new<T: RandomSource>(rand: &mut T) -> ImprovedNoise {
        let pos = DVec3::new(rand.next_f64(), rand.next_f64(), rand.next_f64()) * 256.0;

        let mut permutation_table: [u8; 256] = std::array::from_fn(|i| i as u8);
        for i in 0..256 {
            let offset = rand.next_u32_bounded(256 - i as u32) as usize;
            permutation_table.swap(i, i + offset);
        }
    ...
    fn sample_and_lerp(&self, pos: ISizeVec3, pos_r: DVec3, yr_original: f64) -> f64 {
        let x: [usize; 2] = std::array::from_fn(|i| {
            self.permutation_table[i.wrapping_add_signed(pos.x) & 0xFF] as usize
        });
        let xy: [usize; 4] = std::array::from_fn(|i| {
            self.permutation_table[(x[i & 1] + (i >> 1)).wrapping_add_signed(pos.y) & 0xFF] as usize
        });
        let [d000, d001, d010, d011, d100, d101, d110, d111] = std::array::from_fn(|i| {
            Self::grad_dot(
                self.permutation_table[(xy[i & 3] + (i >> 2)).wrapping_add_signed(pos.z) & 0xFF]
                    as usize,
